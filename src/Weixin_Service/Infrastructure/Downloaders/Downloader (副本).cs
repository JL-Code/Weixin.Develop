...
namespace JCode.Infrastructure.Downloaders
{
    ...
    /// 文件下载工具类 带进度提示
    /// </summary>
    public class Downloader备份 : IDisposable
    {
        ...
        /// <param name="to">结束位置</param>
        /// <returns></returns>
        public async Task DownloadAsync(long from, long to)
        {
            if (_totalSize == 0)
                GetTotalSize();
            if (_currentSize >= _totalSize)
            ...
            using (var bytesRep = response.Content.ReadAsByteArrayAsync())
            {
                var fileByteArr = await bytesRep;
                _currentSize += fileByteArr.Length;
                await _fs.WriteAsync(fileByteArr, 0, fileByteArr.Length);
            };
            //不支持断点下载
            if (response.Content.Headers.ContentRange == null)
            {
